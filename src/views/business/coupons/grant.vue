<template>
    <div class="whiteBg">
        <van-nav-bar class="navBar" title="券发放记录" left-text="返回" left-arrow @click-left="onClickLeft" fixed/>
                            ...
                            <van-card  class="vanCard" v-for="(item,index) in tableData" :key="index">
                                <template #title>
                                    <van-row type="flex" justify="center" >
                                        ...
                                        <van-col span="12" class="colInfo">发放数量：{{item.quotaNum}}张</van-col>
                                        <van-col span="12" class="colInfo">操作人：{{item.merchantsName}}</van-col>
                                    </van-row>
                                </template>
                            </van-card>
                        </van-tab>
                        <van-tab title="领取记录"  name="2">
                            <van-card  class="vanCard" v-for="(item,index) in drawData" :key="index">
                                <template #title>
                                    <van-row type="flex" justify="center" >
                                        <van-col span="24" class="colInfo">
                                            <h2>车牌:{{item.bindCar}}</h2>
                                        </van-col>
                                        <van-col span="12" class="colInfo">领取时间：{{item.bindTime}}</van-col>
                                        <van-col span="12" class="colInfo">过期时间：{{item.invalidTime}}</van-col>
                                        <van-col span="12" class="colInfo">券类型：
                                            <span v-if="item.quotaType==1">{{item.quotaName}}券</span>
                                            <span v-if="item.quotaType==2">时长券</span>
                                        ...
                                        <van-col span="24" class="colInfo">操作人：{{item.merchantsName}}</van-col>
                                    </van-row>
                                </template>
                            </van-card>
                        </van-tab>
...
export default {
    ...
    methods:{
        ...
        init(){
            ...
            this.$api.business.quotaGrantList(this.params).then(res=>{
                if(res.code == 200){
                    //debugger
                    this.loading = false;
        ...
        getDrawDataList(){
            ...
            this.$api.business.quotaBindList(this.params).then(res=>{
                if(res.code == 200){
                    //debugger
                    this.loading = false;
                    this.total = res.result.total
                    ...
                    this.drawData = this.drawData.concat(rows)
                    //如果列表数据条数>=总条数，不再触发滚动加载
                    if (this.drawData.length >= this.total) {
                        this.finished = true;
                    }                    
                    this.pageNo++;
                }else{
